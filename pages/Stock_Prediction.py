...
# Basic validation to avoid empty arrays causing errors downstream
if close_price is None or len(close_price) == 0:
    st.error(
        "No price data was found for this ticker. "
...
# Ensure we have enough data after rolling-mean preprocessing
if len(rolling_price) < 60:
    st.error(
        "Not enough historical data after preprocessing to train the forecast model. "
        "Please try a different ticker with more price history."
    )
    st.stop()
